package Font::OTF::Glyph;

=head1 NAME

Font::OTF::Glyph - Holds a information for a single glyph
...
=head1 Font::OTF::Glyph->new(%parms)
...
sub read_dat {
    ...
    if ($num > 0) {
        ...
        $fp += $num << 1;
        $max = 0;
        for (@{$self->{'endPoints'}}) { $max = $_ if $_ > $max; }
...
#        Font::OTF::Utils::XML_hexdump($context, $depth . $context->{'indent'}, $self->{'hints'});
        $dat = Font::OTF::Utils::XML_binhint($self->{'hints'}) || "";
        $dat =~ s/\n(?!$)/\n$depth$context->{'indent'}/mg;
        $fh->print("$depth$context->{'indent'}$dat");
        $fh->print("$depth</hints>\n");
    }
    else { return Font::OTF::Table::XML_element(@_); }
...
sub update {
    ...
    if ($num > 0) {
        ...
        for ($i = 0; $i < $self->{'numPoints'}; $i++) {
            ...
            if (($flag & 18) == 0) { $self->{' DAT'} .= TTF_Pack("s", $x); }
            elsif (($flag & 18) == 18) { $self->{' DAT'} .= pack("C", $x); }
            elsif (($flag & 18) == 2) { $self->{' DAT'} .= pack("C", -$x); }
        }
        for ($i = 0; $i < $self->{'numPoints'}; $i++) {
            $flag = $self->{'flags'}[$i];
            $y = $self->{'y'}[$i] - (($i == 0) ? 0 : $self->{'y'}[$i - 1]);
            if (($flag & 36) == 0) { $self->{' DAT'} .= TTF_Pack("s", $y); }
            elsif (($flag & 36) == 36) { $self->{' DAT'} .= pack("C", $y); }
            elsif (($flag & 36) == 4) { $self->{' DAT'} .= pack("C", -$y); }
        }
    }
        ...
        for ($i = 0; $i <= $#{$self->{'comps'}}; $i++) {
            ...
            if ($flag & 1) { $self->{' DAT'} .= TTF_Pack("s2", @{$comp->{'args'}}); }
            else { $self->{' DAT'} .= pack("CC", @{$comp->{'args'}}); }

            if ($flag & 8) { $self->{' DAT'} .= TTF_Pack("F", $comp->{'scale'}[0]); }
            elsif ($flag & 64) {
            	$self->{' DAT'} .= TTF_Pack("F2", $comp->{'scale'}[0], $comp->{'scale'}[3]);
            }
            elsif ($flag & 128) { $self->{' DAT'} .= TTF_Pack("F4", @{$comp->{'scale'}}); }
        }
...
sub get_points {
    ...
    for $comp (@{$self->{'comps'}}) {
        ...
        for $e (@{$compg->{'endPoints'}}) { push (@{$self->{'endPoints'}}, $e + $nump); }
