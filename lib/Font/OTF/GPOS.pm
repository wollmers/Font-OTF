package Font::OTF::GPOS;

=head1 NAME

Font::OTF::GPOS - Support for Opentype GPOS tables in conjunction with TTOpen
...
structure of a GPOS is shared with a GSUB table via the L<Font::OTF::Ttopen>
...
sub read_sub {
    ...
    if ($type == 1 && $fmt == 1) {
        ...
        for ($i = 0; $i < $mcount; $i++) {
        	push (@{$lookup->{'RULES'}},
        	   [{'ACTION' => [$self->read_value($count, $loc, $lookup, $fh)]}]
            );
        }
...
sub out_sub {
    ...
    if ($type == 1 && $fmt == 1) {
        $out = pack('n2', $fmt, Font::OTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2 + $base));
        $vfmt = $self->fmt_value($lookup->{'ADJUST'});
        $out .= pack('n', $vfmt) . $self->out_value($lookup->{'ADJUST'}, $vfmt, $ctables, 6 + $base);
    }
    elsif ($type == 1 && $fmt == 2) {
        $vfmt = 0;
        for $r (@{$lookup->{'RULES'}}) { $vfmt |= $self->fmt_value($r->[0]{'ACTION'}[0]); }
        $out = pack('n4', $fmt, Font::OTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2 + $base),
                            $vfmt, $#{$lookup->{'RULES'}} + 1);
        for $r (@{$lookup->{'RULES'}}) {
        ...
        if ($fmt == 1) {
            ...
            $out = pack('n5',
                        $fmt,
                        Font::OTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2 + $base),
                        $vfmt,
                        $vfmt2,
            ...
            my $pairsets = '';
            my (%cache);
            for $r (@{$lookup->{'RULES'}}) { # for PairSet table

                # write offset to this PairSet at end of PairPosFormat1 table
                if (defined $cache{"$r"}) { $out .= pack('n', $cache{"$r"}); }
                else {
                    $out .= pack('n', $off);
                    $cache{"$r"} = $off;
            ...
            $out = pack('n8', $fmt, Font::OTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2 + $base),
                            $vfmt, $vfmt2,
                            Font::OTF::Ttopen::ref_cache($lookup->{'CLASS'}, $ctables, 8 + $base),
                            Font::OTF::Ttopen::ref_cache($lookup->{'MATCH'}[0], $ctables, 10 + $base),
        ...
        $out = pack('n3', $fmt, Font::OTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2 + $base),
                            $#{$lookup->{'RULES'}} + 1);
        for $r (@{$lookup->{'RULES'}}) {
            $out .= pack('n2', Font::OTF::Ttopen::ref_cache($r->[0]{'ACTION'}[0], $ctables, length($out) + $base),
                            Font::OTF::Ttopen::ref_cache($r->[0]{'ACTION'}[1], $ctables, length($out) + 2 + $base));
        }
    }
    elsif ($type == 4 || $type == 5 || $type == 6) {
        my ($loc_off, $loc_t, $ltables);

        $out = pack('n7', $fmt, Font::OTF::Ttopen::ref_cache($lookup->{'MATCH'}[0], $ctables, 2 + $base),
                            Font::OTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 4 + $base),
                            $#{$lookup->{'RULES'}[0][0]{'ACTION'}} + 1, 12, ($#{$lookup->{'MARKS'}} + 4) << 2,
                            $#{$lookup->{'MARKS'}} + 1);
        for $r (@{$lookup->{'MARKS'}}) { $out .= pack('n2', $r->[0], Font::OTF::Ttopen::ref_cache($r->[1], $mtables, length($out) + 2)); }
        ...
        for ($i = 0; $i <= $#{$lookup->{'RULES'}}; $i++) {
            ...
            for $t (@$r) {
                for $s (@{$t->{'ACTION'}}) { $out .= pack('n', Font::OTF::Ttopen::ref_cache($s, $ltables, length($out))); }
            }
            push (@reftables, [$ltables, $loc_t]) if ($type == 5);
        }
        push (@reftables, [$ltables, $loc_t]) unless ($type == 5);
        $out = Font::OTF::Ttopen::out_final($fh, $out, \@reftables, 1);
    }
    elsif ($type == 7 || $type == 8) {
...
sub read_value {
    ...
    $fh->read($dat, $s << 1);
    $flag = 1; $i = 0;
    for $s (qw(XPlacement YPlacement XAdvance YAdvance)) {
        $res->{$s} = TTF_Unpack('s', substr($dat, $i++ << 1, 2)) if ($fmt & $flag);
        $flag <<= 1;
    }

    for $s (qw(XPlaDevice YPlaDevice XAdvDevice YAdvDevice)) {
        if ($fmt & $flag) {
    ...
    for $s (qw(XIdPlacement YIdPlacement XIdAdvance YIdAdvance)) {
        $res->{$s} = TTF_Unpack('S', substr($dat, $i++ << 1, 2)) if ($fmt & $flag);
        $flag <<= 1;
    }
    $res;
...
sub read_delta {
    ...
    $res = Font::OTF::Delta->new->read($fh);
    $fh->seek($loc, 0);
...
sub read_anchor {
    ...
    return $lookup->{' CACHE'}{$str} if defined $lookup->{' CACHE'}{$str};
    $fh->seek($base + $offset, 0);
    $res = Font::OTF::Anchor->new->read($fh);
    $fh->seek($loc, 0);
    $lookup->{' CACHE'}{$str} = $res;
    return $res;
}
...
sub fmt_value {
    ...
    for my $n (reverse qw(XPlacement YPlacement XAdvance YAdvance XPlaDevice YPlaDevice
...
sub out_value {
    ...
    $flag = 1;
    for $n (qw(XPlacement YPlacement XAdvance YAdvance)) {
        $out .= pack('n', $value->{$n}) if ($flag & $fmt);
        $flag <<= 1;
    }
    for $n (qw(XPlaDevice YPlaDevice XAdvDevice YAdvDevice)) {
        if ($flag & $fmt) {
            $out .= pack('n', Font::OTF::Ttopen::ref_cache(
                        $value->{$n}, $tables, $offset + length($out)));
        }
        $flag <<= 1;
    }
    for $n (qw(XIdPlacement YIdPlacement XIdAdvance YIdAdvance)) {
        $out .= pack('n', $value->{$n}) if ($flag & $fmt);
        $flag <<= 1;
    }
    $out;
